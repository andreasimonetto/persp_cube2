...
/* old routines */
/*
extern ply_file_t *__ply_write(FILE *, int, char **, int);
extern ply_file_t *__ply_read(FILE *, int *, char ***);
extern ply_file_t *__ply_open_for_reading( char *, int *, char ***, int *, float *);
extern void __ply_close(ply_file_t *);
extern ply_other_prop_t *__ply_get_other_properties(ply_file_t *, char *, int);
extern void __ply_describe_property(ply_file_t *, char *, ply_property_t *);
extern void ply_get_property(ply_file_t *, char *, ply_property_t *);
extern void __ply_get_element(ply_file_t *, void *);
*/

/* find an element in a plyfile's list */
static ply_element_t *find_element(ply_file_t *, char *);

/* find a property in an element's list */
static ply_property_t *find_property(ply_element_t *, char *, int *);
...
static void add_element(ply_file_t *, char **, int);
static void add_property(ply_file_t *, char **, int);
static void add_comment(ply_file_t *, char *);
static void add_obj_info(ply_file_t *, char *);

/* copy a property */
static void copy_property(ply_property_t *, ply_property_t *);
...
static void ascii_get_element(ply_file_t *, char *);
static void binary_get_element(ply_file_t *, char *);
...
Exit:
  returns a pointer to a ply_file_t, used to refer to this file, or NULL if error
******************************************************************************/

ply_file_t *__ply_write(
  FILE *fp,
  int nelems,
  char **elem_names,
  int file_type
)
{
  ...
  plyfile = (ply_file_t *) secure_alloc (sizeof (ply_file_t));
  ...
  plyfile->elems = (ply_element_t **) secure_alloc (sizeof (ply_element_t *) * nelems);
  for (i = 0; i < nelems; i++) {
    elem = (ply_element_t *) secure_alloc (sizeof (ply_element_t));
...
ply_file_t *ply_open_for_writing(
  char *filename,
  int nelems,
  char **elem_names,
  int file_type
)
{
  ...
  name = (char *) secure_alloc (sizeof (char) * (strlen (filename) + 5));
  ...
  /* create the actual ply_file_t structure */

  plyfile = __ply_write (fp, nelems, elem_names, file_type);
  if (plyfile == NULL)
    return (NULL);
...
void ply_element_layout(
  ply_file_t *plyfile,
  char *elem_name,
  int nelems,
  int nprops,
  ply_property_t *prop_list
)
{
  int i;
  ply_element_t *elem;
  ...
  if (elem == NULL) {
    fprintf(stderr,"ply_element_layout: can't find element '%s'\n",elem_name);
    exit (-1);
  }
  ...
  elem->nprops = nprops;
  elem->props = (ply_property_t **) secure_alloc (sizeof (ply_property_t *) * nprops);
  elem->store_prop = (char *) secure_alloc (sizeof (char) * nprops);

  for (i = 0; i < nprops; i++) {
    prop = (ply_property_t *) secure_alloc (sizeof (ply_property_t));
    elem->props[i] = prop;
...
)
{
  ...
  if (elem->nprops == 0) {
    elem->props = (ply_property_t **) secure_alloc (sizeof (ply_property_t *));
    elem->store_prop = (char *) secure_alloc (sizeof (char));
    elem->nprops = 1;
  }
  else {
    elem->nprops++;
    elem->props = (ply_property_t **)
                  realloc (elem->props, sizeof (ply_property_t *) * elem->nprops);
  ...
  elem_prop = (ply_property_t *) secure_alloc (sizeof (ply_property_t));
...
void ply_header_complete(ply_file_t *plyfile)
{
  int i,j;
  FILE *fp = plyfile->fp;
  ply_element_t *elem;
...
void ply_element_setup(ply_file_t *plyfile, char *elem_name)
{
  ply_element_t *elem;

  elem = find_element (plyfile, elem_name);
  if (elem == NULL) {
    fprintf(stderr, "ply_element_setup: can't find element '%s'\n", elem_name);
    exit (-1);
  }
...
void ply_put_element(ply_file_t *plyfile, void *elem_ptr)
{
  int j,k;
  FILE *fp = plyfile->fp;
  ply_element_t *elem;
...
ply_file_t *__ply_read(FILE *fp, int *nelems, char ***elem_names)
{
  int i,j;
  ply_file_t *plyfile;
  int nwords;
  ...
  while (words) {
    ...
    if (!strcmp (words[0], "format")) {
      ...
      else if (!strcmp (words[1], "binary_little_endian"))
    ...
    else if (!strcmp (words[0], "comment"))
      add_comment (plyfile, orig_line);
    else if (!strcmp (words[0], "obj_info"))
  ...
  for (i = 0; i < plyfile->num_elem_types; i++) {
    elem = plyfile->elems[i];
    elem->store_prop = (char *) secure_alloc (sizeof (char) * elem->nprops);
    for (j = 0; j < elem->nprops; j++)
  ...
  elist = (char **) secure_alloc (sizeof (char *) * plyfile->num_elem_types);
  for (i = 0; i < plyfile->num_elem_types; i++)
...
ply_file_t *ply_open_for_reading(
  char *filename,
  int *nelems,
  char ***elem_names,
  int *file_type,
...
)
{
  ...
  /* create the ply_file_t data structure */

  plyfile = __ply_read (fp, nelems, elem_names);
...
ply_property_t **ply_get_element_description(
  ply_file_t *plyfile,
  char *elem_name,
  int *nelems,
  int *nprops
)
{
  ...
  *nprops = elem->nprops;
  ...
  prop_list = (ply_property_t **) secure_alloc (sizeof (ply_property_t *) * elem->nprops);
...
void __ply_get_element(ply_file_t *plyfile, void *elem_ptr)
{
  if (plyfile->file_type == PLY_ASCII)
...
char **ply_get_comments(ply_file_t *plyfile, int *num_comments)
{
...
char **ply_get_obj_info(ply_file_t *plyfile, int *num_obj_info)
{
...
void setup_other_props(ply_file_t *plyfile, ply_element_t *elem)
{
  int i;
  ply_property_t *prop;
  int size = 0;
...
static ply_other_prop_t *get_other_properties(
  ply_file_t *plyfile,
  ply_element_t *elem,
  int offset
)
{
  ...
  other = (ply_other_prop_t *) secure_alloc (sizeof (ply_other_prop_t));
  ...
  other->props = (ply_property_t **) secure_alloc (sizeof(ply_property_t) * elem->nprops);
...
ply_other_prop_t *__ply_get_other_properties(
  ply_file_t *plyfile,
  char *elem_name,
  int offset
)
{
  ...
  elem = find_element (plyfile, elem_name);
  if (elem == NULL) {
    fprintf (stderr, "__ply_get_other_properties: Can't find element '%s'\n",
             elem_name);
    return (NULL);
  }
...
ply_other_elems_t *ply_get_other_element (ply_file_t *plyfile)
{
  int i;
  ply_element_t *elem;
  char *elem_name;
  int elem_count;
  ply_other_elems_t *other_elems;
  ply_other_elem_t *other;
  ...
  if (plyfile->other_elems == NULL) {
    plyfile->other_elems = (ply_other_elems_t *) secure_alloc (sizeof (ply_other_elems_t));
    other_elems = plyfile->other_elems;
    other_elems->other_list = (ply_other_elem_t *) secure_alloc (sizeof (ply_other_elem_t));
    other = &(other_elems->other_list[0]);
    other_elems->num_elems = 1;
  }
  else {
    other_elems = plyfile->other_elems;
    other_elems->other_list = (ply_other_elem_t *) realloc (other_elems->other_list,
                              sizeof (ply_other_elem_t) * other_elems->num_elems + 1);
  ...
  other->other_data = (ply_other_data_t **)
                  malloc (sizeof (ply_other_data_t *) * other->elem_count);

  /* set up for getting elements */
  other->other_props = __ply_get_other_properties (plyfile, elem_name,
                         offsetof(ply_other_data_t,other_props));

  /* grab all these elements */
  for (i = 0; i < other->elem_count; i++) {
    /* grab and element from the file */
    other->other_data[i] = (ply_other_data_t *) malloc (sizeof (ply_other_data_t));
    __ply_get_element (plyfile, (void *) other->other_data[i]);
  }
...
void ply_put_other_elements (ply_file_t *plyfile)
{
  int i,j;
  ply_other_elem_t *other;
  ...
  for (i = 0; i < plyfile->other_elems->num_elems; i++) {
    ...
    ply_element_setup (plyfile, other->elem_name);
...
void ply_free_other_elements (ply_other_elems_t *other_elems)
{
...
void __ply_close(ply_file_t *plyfile)
{
  fclose (plyfile->fp);
...
/******************************************************************************
Get version number and file type of a ply_file_t.
...
void ply_get_info(ply_file_t *ply, float *version, int *file_type)
{
  if (ply == NULL)
    return;
...
static ply_element_t *find_element(ply_file_t *plyfile, char *element)
{
  int i;

  for (i = 0; i < plyfile->num_elem_types; i++)
    if (!strcmp (element, plyfile->elems[i]->name))
...
static ply_property_t *find_property(ply_element_t *elem, char *prop_name, int *index)
{
  int i;

  for (i = 0; i < elem->nprops; i++)
    if (!strcmp (prop_name, elem->props[i]->name)) {
      *index = i;
      return (elem->props[i]);
    }
...
static void ascii_get_element(ply_file_t *plyfile, char *elem_ptr)
{
  int j,k;
  ply_element_t *elem;
  ply_property_t *prop;
  ...
  if (elem->other_offset != NO_OTHER_PROPS) {
    ...
    other_data = (char *) secure_alloc (elem->other_size);
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list == PLY_LIST) {       /* a list */
      ...
      if (list_count == 0) {
        ...
        if (store_it) {
          item_ptr = (char *) secure_alloc (sizeof (char) * item_size * list_count);
          item = item_ptr;
...
static void binary_get_element(ply_file_t *plyfile, char *elem_ptr)
{
  int j,k;
  ply_element_t *elem;
  ply_property_t *prop;
...
static void write_scalar_type (FILE *fp, int code)
{
...
static char **get_words(FILE *fp, int *nwords, char **orig_line)
{
  ...
  words = (char **) secure_alloc (sizeof (char *) * max_words);
...
******************************************************************************/
/*
static double get_item_value(char *item, int type)
{
...
static void add_element (ply_file_t *plyfile, char **words, int nwords)
{
  ply_element_t *elem;
  ...
  if (plyfile->num_elem_types == 0)
    plyfile->elems = (ply_element_t **) secure_alloc (sizeof (ply_element_t *));
  else
    plyfile->elems = (ply_element_t **) realloc (plyfile->elems,
                     sizeof (ply_element_t *) * (plyfile->num_elem_types + 1));
...
int get_prop_type(char *type_name)
{
  ...
  for (i = StartType + 1; i < EndType; i++)
    if (!strcmp (type_name, old_type_names[i]))
      return (i);
...
static void add_property (ply_file_t *plyfile, char **words, int nwords)
{
  ply_property_t *prop;
  ply_element_t *elem;
  ...
  if (!strcmp (words[1], "list")) {          /* list */
    prop->count_external = get_prop_type (words[2]);
    prop->external_type = get_prop_type (words[3]);
    prop->name = strdup (words[4]);
    prop->is_list = PLY_LIST;
  }
  else if (!strcmp (words[1], "string")) {   /* string */
  ...
  if (elem->nprops == 0)
    ...
    elem->props = (ply_property_t **) realloc (elem->props,
                  sizeof (ply_property_t *) * (elem->nprops + 1));
...
static void add_comment (ply_file_t *plyfile, char *line)
{
  int i;
  ...
  ply_append_comment (plyfile, &line[i]);
}
...
static void add_obj_info (ply_file_t *plyfile, char *line)
{
  int i;
  ...
  ply_append_obj_info (plyfile, &line[i]);
}
...
static void copy_property(ply_property_t *dest, ply_property_t *src)
{
...
ply_file_t *ply_read(FILE *fp)
{
  ply_file_t *ply;
  int num_elems;
  char **elem_names;

  ply = __ply_read (fp, &num_elems, &elem_names);
...
)
{
  ...
  ply = __ply_write (fp, nelems, elem_names, file_type);
...
char **ply_get_element_list(ply_file_t *ply, int *num_elems)
{
  int i;
  char **elist;
  ...
  elist = (char **) secure_alloc (sizeof (char *) * ply->num_elem_types);
...
void ply_append_comment(ply_file_t *ply, char *comment)
{
  /* (re)allocate space for new comment */
  if (ply->num_comments == 0)
    ply->comments = (char **) secure_alloc (sizeof (char *));
  else
...
void ply_copy_comments(ply_file_t *out_ply, ply_file_t *in_ply)
{
  int i;

  for (i = 0; i < in_ply->num_comments; i++)
    ply_append_comment (out_ply, in_ply->comments[i]);
}
...
void ply_append_obj_info(ply_file_t *ply, char *obj_info)
{
  /* (re)allocate space for new info */
  if (ply->num_obj_info == 0)
    ply->obj_info = (char **) secure_alloc (sizeof (char *));
  else
...
void ply_copy_obj_info(ply_file_t *out_ply, ply_file_t *in_ply)
{
  int i;

  for (i = 0; i < in_ply->num_obj_info; i++)
    ply_append_obj_info (out_ply, in_ply->obj_info[i]);
}
...
void ply_close(ply_file_t *plyfile)
{
  fclose (plyfile->fp);
}
...
void ply_free(ply_file_t *plyfile)
{
  /* free up memory associated with the PLY file */
  free (plyfile);
}
...
char *ply_setup_element_read (ply_file_t *ply, int index, int *elem_count)
{
  ply_element_t *elem;
...
void ply_get_element (ply_file_t *plyfile, void *elem_ptr)
{
  if (plyfile->file_type == PLY_ASCII)
    ascii_get_element (plyfile, (char *) elem_ptr);
  else
...
call to the routine ply_get_element().
...
void ply_setup_property(ply_file_t *plyfile, const ply_property_t *prop)
{
ply_element_t *elem;
ply_property_t *prop_ptr;
int index;
...
ply_other_prop_t *ply_get_other_properties(
  ply_file_t *plyfile,
  int offset
)
{
...
)
{
  ...
  if (elem->nprops == 0) {
    elem->props = (ply_property_t **)
                  secure_alloc (sizeof (ply_property_t *) * other->nprops);
    elem->store_prop = (char *) secure_alloc (sizeof (char) * other->nprops);
    elem->nprops = 0;
  }
  else {
    int newsize;
    ...
    elem->props = (ply_property_t **)
                  realloc (elem->props, sizeof (ply_property_t *) * newsize);
    elem->store_prop = (char *)
...
ply_prop_rules_t *ply_init_rule (ply_file_t *ply, char *elem_name)
{
  int i,j;
  ply_element_t *elem;
  ply_prop_rules_t *rules;
  ply_rule_list_t *list;
  int found_prop;

  elem = find_element (ply, elem_name);
  if (elem == NULL) {
    fprintf (stderr, "ply_init_rule: Can't find element '%s'\n", elem_name);
    exit (-1);
  }

  rules = (ply_prop_rules_t *) secure_alloc (sizeof (ply_prop_rules_t));
  rules->elem = elem;
  rules->rule_list = (int *) secure_alloc (sizeof(int) * elem->nprops);
  rules->max_props = 0;
  rules->nprops = 0;
  ...
  for (list = ply->rule_list; list != NULL; list = list->next) {
    ...
    if (!!strcmp (list->element, elem->name))
      continue;

    found_prop = 0;

    for (i = 0; i < elem->nprops; i++)
      if (!strcmp (list->property, elem->props[i]->name)) {

        found_prop = 1;

        /* look for matching rule name */
        for (j = 0; rule_name_list[j].code != -1; j++)
          if (!strcmp (list->name, rule_name_list[j].name)) {
...
void ply_modify_rule (ply_prop_rules_t *rules, char *prop_name, int rule_type)
{
  int i;
  ply_element_t *elem = rules->elem;

  /* find the property and modify its rule type */

  for (i = 0; i < elem->nprops; i++)
    if (!strcmp (elem->props[i]->name, prop_name)) {
      rules->rule_list[i] = rule_type;
      return;
    }

  /* we didn't find the property if we get here */
  fprintf (stderr, "ply_modify_rule: Can't find property '%s'\n", prop_name);
  exit (-1);
}
...
void ply_start_props (ply_file_t *ply, ply_prop_rules_t *rules)
{
...
void ply_weight_props (ply_file_t *ply, float weight, void *other_props)
{
  ply_prop_rules_t *rules = ply->current_rules;

  /* allocate space for properties and weights, if necessary */
  if (rules->max_props == 0) {
    rules->max_props = 6;
    rules->props = (void **) secure_alloc (sizeof (void *) * rules->max_props);
    rules->weights = (float *) secure_alloc (sizeof (float) * rules->max_props);
  }
...
void *ply_get_new_props(ply_file_t *ply)
{
  int i,j;
  static double *vals;
  ...
  new_data = (char *) secure_alloc (sizeof (char) * elem->other_size);

  /* make sure there is enough room to store values we're to combine */

  if (max_vals == 0) {
    max_vals = rules->nprops;
    vals = (double *) secure_alloc (sizeof (double) * rules->nprops);
  }
  ...
  for (i = 0; i < elem->nprops; i++) {
	...
	for (j = 1; j < rules->nprops; j++)
	  if (double_val != vals[j]) {
	    fprintf (stderr,
    "ply_get_new_props: Error combining properties that should be the same.\n");
            exit (-1);
	  }
        break;
      }
      default:
        fprintf (stderr, "ply_get_new_props: Bad rule = %d\n",
	         rules->rule_list[i]);
	exit (-1);
    }
...
void ply_set_prop_rules (ply_file_t *ply, ply_rule_list_t *prop_rules)
{
...
ply_rule_list_t *ply_append_prop_rule (
  ply_rule_list_t *rule_list,
  char *name,
  char *property
)
{
  ply_rule_list_t *rule;
  ply_rule_list_t *rule_ptr;
  char *str,*str2;
  char *ptr;
  ...
  rule = (ply_rule_list_t *) malloc (sizeof (ply_rule_list_t));
  rule->name = name;
  rule->element = str;
  rule->property = str2;
  rule->next = NULL;
...
int ply_matches_rule_name (char *name)
{
  int i;

  for (i = 0; rule_name_list[i].code != -1; i++)
    if (!strcmp (rule_name_list[i].name, name))
      return (1);
